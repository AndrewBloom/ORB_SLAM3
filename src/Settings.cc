...
namespace ORB_SLAM3 {
    ...
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false) {
        ...
        if(sensor_ == OrbSlam3::STEREO || sensor_ == OrbSlam3::IMU_STEREO){
        ...
        if(sensor_ == OrbSlam3::IMU_MONOCULAR || sensor_ == OrbSlam3::IMU_STEREO || sensor_ == OrbSlam3::IMU_RGBD){
            readIMU(fSettings);
            cout << "\t-Loaded IMU calibration" << endl;
        }

        if(sensor_ == OrbSlam3::RGBD || sensor_ == OrbSlam3::IMU_RGBD){
            readRGBD(fSettings);
    ...
    void Settings::readCamera1(cv::FileStorage &fSettings) {
        ...
        if (cameraModel == "PinHole") {
            ...
            if((sensor_ == OrbSlam3::MONOCULAR || sensor_ == OrbSlam3::IMU_MONOCULAR) && vPinHoleDistorsion1_.size() != 0){
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings) {
        ...
        if(found){
            ...
            if(!bNeedToRectify_){
                ...
                if((sensor_ == OrbSlam3::STEREO || sensor_ == OrbSlam3::IMU_STEREO) && cameraType_ != Rectified){
    ...
    void Settings::precomputeRectificationMaps() {
        ...
        if(sensor_ == OrbSlam3::IMU_STEREO){
    ...
    ostream &operator<<(std::ostream& output, const Settings& settings){
        ...
        if(settings.sensor_ == OrbSlam3::STEREO || settings.sensor_ == OrbSlam3::IMU_STEREO){
        ...
        if(settings.sensor_ == OrbSlam3::IMU_MONOCULAR || settings.sensor_ == OrbSlam3::IMU_STEREO || settings.sensor_ == OrbSlam3::IMU_RGBD) {
        ...
        if(settings.sensor_ == OrbSlam3::RGBD || settings.sensor_ == OrbSlam3::IMU_RGBD){
